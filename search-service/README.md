...
- 작품명과 계절 표현의 혼동 방지
- 장소·작품·지역·도시·계절·시간대의 확실한 조건만 하드 필터로 적용
- 검수 완료 테마 매핑을 `source_segment_id` 기준 하드 필터로 적용
- UI 선택 필터를 QueryParser 결과보다 우선 적용
- 감성·활동·장면 요소를 검색 후보 제거가 아닌 소프트 힌트로 사용
- CLIP 텍스트 질의 임베딩 생성과 모델 1회 로딩·재사용
- PostgreSQL/pgvector의 텍스트·이미지 임베딩 검색
- 텍스트·이미지 순위의 RRF 결합
- 동일 `source_segment_id`의 최고 SCENE을 대표 결과로 선택한 뒤 Top-K 적용
- 유사도 분포를 반영한 정규화 점수 결합
- Hit@K, Recall@K, MRR, nDCG@K 및 단계별 응답시간 측정
- 실제 구간 JSON과 VLM 메타데이터의 스키마·ID·시간·경로 검증
- 검색 실패 사례 기록과 원인 분류

실제 `/api/search` 자연어 입력 필드는 `q`를 사용합니다. `q`가 비어 있어도
테마·계절·작품·지역 등 구조화 필터가 있으면 OpenAI 호출 없이 검색할 수 있습니다.
동일 필드의 여러 값은 OR, 서로 다른 필드는 AND로 적용합니다.
...
Windows 임시 폴더 권한 문제를 피하려면 프로젝트 내부의 별도 폴더를 사용합니다.
...
`data/multilingual_theme_evaluation_40.json`은 한국어·영어·일본어·중국어
40문항의 검색 정답셋입니다. 최종 CLIP 임베딩 517건을 대상으로 수행한 오프라인
평가 결과는 `evaluation/actual_search_evaluation.md`에서 확인할 수 있습니다.

| 방식 | Hit@5 | Recall@5 | MRR | nDCG@5 |
|---|---:|---:|---:|---:|
| RRF | 1.0000 | 0.9625 | 0.7933 | 0.8355 |

- 결과 단위: `source_segment_id`
- RRF: `k=60`
- RRF 실패: 0/40
- 평균 검색 처리시간(QueryParser 제외): 30.7ms

이 수치는 프로젝트용으로 구성한 소규모 40문항 평가 결과이며 일반 서비스 전체의
성능을 보장하는 외부 벤치마크로 해석하지 않습니다.
...
- `data/multilingual_theme_evaluation_40.json`: 최종 517개 SCENE용 4개 언어 40문항 평가셋
- `evaluation/actual_search_evaluation.md`: 최종 RRF 검색 평가 요약
