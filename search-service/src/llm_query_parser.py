...
6. 작품 제목에 포함된 단어를 계절·지역·시간 조건으로 오해하지 않습니다.
   예: '겨울연가', 'Winter Sonata', '冬のソナタ', '冬季恋歌' 안의 겨울 표현은 계절 필터가 아닙니다.
7. 작품명 밖에서 계절을 명시한 경우에는 계절 필터를 사용합니다.
   예: '겨울에 겨울연가 촬영지'의 작품명 밖 '겨울에'는 계절 필터입니다.
8. 확신할 수 없는 필드는 빈 목록이 아니라 해당 키 자체를 생략합니다.
9. 계절은 봄/여름/가을/겨울, 시간대는 새벽/아침/낮/해질녘/밤으로 정규화합니다.
10. 지역과 감성은 가능한 경우 한국어 표준 표현으로 정규화합니다.
11. 사용자가 말하지 않은 장소·계절·감성을 새로 만들지 않습니다.
"""
