...
from .query_parser import (
    ...
    parse_query_safely,
    parsed_query_from_explicit_filters,
    to_filter_arguments,
)
from .theme_filters import (
    build_theme_index,
    filter_by_themes,
    normalize_themes,
    theme_search_text,
...
class MultimodalSearchPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self._theme_mapping_configured = theme_mapping is not None
        self._theme_index = build_theme_index(theme_mapping)
    ...
    def search(
        ...
        *,
        ...
        theme: str | Sequence[str] | None = None,
        ...
        if search_depth is not None and candidate_k is not None:
            raise ValueError("search_depth와 candidate_k는 동시에 지정할 수 없습니다.")
        requested_depth = (
            candidate_k
            if candidate_k is not None
            else search_depth if search_depth is not None else max(top_k * 5, 50)
        )
        ...
        normalized_themes = normalize_themes(theme)
        has_natural_query = isinstance(query, str) and bool(query.strip())
        if has_natural_query:
            if parser is None:
                raise ValueError("자연어 query가 있으면 parser가 필요합니다.")
            parsed = parse_query_safely(query, parser)
            parsed = merge_filter_overrides(parsed, filter_overrides)
            parser_skipped = False
        else:
            static_text = theme_search_text(normalized_themes)
            parsed = parsed_query_from_explicit_filters(
                filter_overrides,
                search_text=static_text or "관광지 촬영지",
            )
            parser_skipped = True
        if normalized_themes and not self._theme_mapping_configured:
            raise ValueError("theme 검색에는 검수 완료 theme_mapping이 필요합니다.")
        ...
        candidates = filter_by_themes(
            candidates,
            normalized_themes,
            theme_index=self._theme_index,
        )
        has_explicit_constraints = bool(filter_overrides) or bool(normalized_themes)
        if parsed.filters and not candidates and not has_explicit_constraints:
            parsed = replace(
                parsed,
                filters={},
                fallback_used=True,
        ...
        depth = min(requested_depth, len(candidate_ids))
            ...
            enriched.append(
                {
                    ...
                    **dict(fused),
                    ...
                    "text_rank": dict(fused).get("source_ranks", {}).get("text"),
                    "image_rank": dict(fused).get("source_ranks", {}).get("image"),
                    "final_score": _result_score(fused),
                }
            )
        return enriched
...
def collapse_results_by_source_segment(
    ...
    *,
    ...
    for index, item in enumerate(results):
        ...
        text_score = _optional_score(item, "text_score", fallback=None)

        # RRF가 같으면 이미지 원점수, 텍스트 원점수, segment_id 순으로 고른다.
        selection_key = (
            final_score,
            image_score if image_score is not None else -math.inf,
            text_score if text_score is not None else -math.inf,
        )
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            -_result_score(item),
            -(_optional_score(item, "image_score", fallback=None) or -math.inf),
            -(_optional_score(item, "text_score", fallback=None) or -math.inf),
            str(item["segment_id"]),
        ),
    )[:top_k]
    return [
