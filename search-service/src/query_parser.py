...
_VALUE_ALIASES: Mapping[str, Mapping[str, tuple[str, ...]]] = {
    ...
    "time_of_day": {
        ...
        "밤": ("밤", "야간", "야경", "night", "nighttime", "夜", "夜晚"),
    },
    "mood": {
        "고요한": (
...
class RuleBasedQueryParser:
    ...
    _SCALAR_KEYWORDS: Mapping[str, tuple[str, ...]] = {
        ...
        "season": ("봄", "여름", "가을", "겨울"),
        "time_of_day": ("새벽", "아침", "낮", "해질녘", "밤", "야간", "야경"),
    }
...
def merge_filter_overrides(
    ...
    """UI가 명시한 하드 필터를 QueryParser 결과보다 우선해 병합한다.

    ``None``·빈 문자열·빈 배열은 '필터 미적용'으로 버린다. 동일 필드의
    여러 값은 목록으로 유지되어 OR로 처리되고 서로 다른 필드는
    ``filter_segments``에서 AND로 처리된다.
    """

    if not overrides:
        return parsed
    cleaned: dict[str, list[str]] = {}
    for field_name, raw_values in overrides.items():
        if field_name not in ALLOWED_FILTER_FIELDS:
            raise ValueError(f"지원하지 않는 필터입니다: {field_name}")
        if raw_values is None:  # type: ignore[comparison-overlap]
            continue
        values: Sequence[str]
        if isinstance(raw_values, str):
            ...
            raise TypeError(f"filter_overrides.{field_name}는 문자열 목록이어야 합니다.")
        nonempty = [value.strip() for value in values if isinstance(value, str) and value.strip()]
        if field_name == "region":
            expanded: list[str] = []
            for value in nonempty:
                location = analyze_locations(value)
                expanded.extend(location.region_filters or (value,))
            nonempty = expanded
        if nonempty:
            cleaned[field_name] = nonempty
    validated = _validate_filter_mapping(cleaned, "filter_overrides")
    return replace(parsed, filters={**parsed.filters, **validated})


def parsed_query_from_explicit_filters(
    filters: Mapping[str, Sequence[str]] | None,
    *,
    search_text: str | None = None,
) -> ParsedQuery:
    """자연어 q가 없는 UI 필터 요청을 OpenAI 호출 없이 검색 구조로 만든다."""

    base = ParsedQuery(
        original_query=(search_text or "필터 조건 관광지 검색").strip(),
        search_text=(search_text or "관광지 촬영지").strip(),
    )
    return merge_filter_overrides(base, filters)
