...
def validate_search_request(payload: object) -> dict[str, Any]:
    ...
    query = _optional_text(payload.get("q", ""), "q")
    ...
    candidate_k = payload.get("candidate_k", max(top_k * 5, 50))
    if (
        isinstance(candidate_k, bool)
        or not isinstance(candidate_k, int)
        or not top_k <= candidate_k <= 1000
    ):
        raise ValueError("candidate_k는 top_k 이상 1000 이하의 정수여야 합니다.")

    result: dict[str, Any] = {
        "q": query,
        "lang": language,
        "theme": _optional_string_list(payload.get("theme"), "theme"),
        "top_k": top_k,
        "candidate_k": candidate_k,
    }
    for field_name in OPTIONAL_REQUEST_FILTERS:
        result[field_name] = _optional_string_list(payload.get(field_name), field_name)
    if not query and not result["theme"] and not any(
        result[field_name] for field_name in OPTIONAL_REQUEST_FILTERS
    ):
        raise ValueError("q 또는 하나 이상의 구조화 필터가 필요합니다.")
    return result
