...
function Invoke-OneVideo {
    Invoke-Preprocess -Arguments @("--manifest", ".\preprocessing_manifest.json", "--output", ".\preprocessed_output", "--rights-confirmed", "--video-id", $VideoId, "--force")
}
...
function Invoke-WithChromeCookie {
    Invoke-Preprocess -Arguments @("--manifest", ".\preprocessing_manifest.json", "--output", ".\preprocessed_output", "--rights-confirmed", "--video-id", $VideoId, "--cookies-from-browser", "chrome", "--force")
}
...
function Reset-ProcessingResults {
    ...
    $Targets = @(
        ...
        (Join-Path $OutputRoot "preprocessed_video"),
    ...
    Write-Host "이전 전처리 결과를 초기화했습니다. original_videos는 유지됩니다."
}
