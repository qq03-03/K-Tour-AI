...
```powershell
Set-Location '<repository-root>\search_automation'
python run_search_sync.py `
...
```powershell
Set-Location '<repository-root>\search_automation'
python run_backend_regression.py `
  --base-url 'https://BACKEND-URL' `
  --evaluation '<path-to-evaluation>\multilingual_theme_evaluation_40.json' `
  --contract-cases 'config\backend_contract_cases.json' `
  --baseline '<path-to-baseline>\previous_backend_regression.json' `
...
```powershell
Set-Location '<repository-root>\search_automation'
python run_local_rule_regression.py `
  --search-service-root '..\search-service' `
  --output 'output\local_rule_regression.json'
```
...
현재 자동화 자체 단위 테스트는 `25 passed`입니다.
