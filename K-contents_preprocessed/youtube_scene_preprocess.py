...
def calculate_frame_metrics(frame: np.ndarray, time_sec: float) -> FrameMetrics:
    ...
    white = ratio((s <= 38) & (v >= 182))
    white_lower = float(
        np.count_nonzero((lower_s <= 38) & (lower_v >= 182))
        / max(1, lower_s.size)
    )
...
def season_scores(metrics: list[FrameMetrics]) -> dict[str, float]:
    ...
    return {season: round(clamp(score), 5) for season, score in scores.items()}
...
def select_candidates(
    ...
    rejected_duplicates: list[SceneCandidate] = []
    ...
    for candidate in accepted_pool:
        ...
        for existing in selected:
            ...
            if similarity >= duplicate_similarity:
                ...
                rejected_duplicates.append(candidate)
                duplicate = True
                break
    ...
    return sorted(selected, key=lambda item: item.start_time), rejected_duplicates
...
def candidate_to_report(candidate: SceneCandidate) -> dict[str, Any]:
...
def process_record(
    ...
    raw_dir = output_root / "raw_videos"
    clips_dir = output_root / "clips" / safe_name(video_id)
    ...
    for index, candidate in enumerate(selected, 1):
        ...
        selected_segments.append({
            ...
            "expected_seasons": expected_seasons,
            "detected_season": candidate.overall_detected_season,
    ...
    for candidate in all_candidates:
        quality_rows.append({
            ...
            "expected_season": candidate.expected_season,
            "season_score": candidate.season_score,
            "detected_season": candidate.overall_detected_season,
...
def main() -> None:
    ...
    for index, record in enumerate(records, 1):
        ...
        if youtube_id in completed_ids:
